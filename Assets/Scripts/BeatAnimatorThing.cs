...
public class BeatAnimatorThing : AudioEventListener
{
	...
	void Start()
	{
		...
		{
			Debug.Log(gameObject.name + ": Couldn't find MeshRenderer");
		}
	...
	void Update()
	{
		...
		if (!_playing || Time.time < lastFrameTime + secsPerFrame / speed || meshRenderer == null) return;
		if (RandomFrame) {
		...
		meshRenderer.material = current.frames[currentFrame];
		lastFrameTime = Time.time;
	}
